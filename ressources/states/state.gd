...
func get_acceleration(xspeed : float, terrain : Terrain, stats : Character) -> float :
	return 2000*get_speed(xspeed, terrain, stats)/get_slopiness(terrain, stats)
...
func process(delta, player : CharacterController, terrain : Terrain, stats : Character):
	...
	if target - xspeed > 0:
		xspeed = min(xspeed + delta*get_acceleration(xspeed, terrain, stats), target)
	if target - xspeed < 0:
		xspeed = max(xspeed - delta*get_acceleration(xspeed, terrain, stats), target)
